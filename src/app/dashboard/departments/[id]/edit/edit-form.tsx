...
const formSchema = z.object({
  ...
  dila: z.string().min(2, { message: "Dila (local jamaat) is required." }),
...
export function EditOfficerForm({ officer }: { officer: IOfficer }) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      positionType: officer.positionType || PositionType.HEAD,
      dila: officer.dila || "",
      userType: officer.userType || UserType.OFFICER,
      isAdmin: officer.isAdmin || false,
      offices: officer.offices || [],
      tenureStart: officer.tenureStart
        ? new Date(officer.tenureStart)
        : new Date(),
  ...
  const updateMutation = useMutation({
    mutationFn: (data: FormValues) => {
      ...
      return officerService.update(officer._id, dto);
    },
    onSuccess: () => {
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="John" className="bg-muted" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Doe" className="bg-muted" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        >
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent>
                      <SelectItem value={PositionType.EXECUTIVE}>
                        Executive (State Qaid, Mut&apos;amad)
                      </SelectItem>
                      <SelectItem value={PositionType.HEAD}>
                        Head (Nazim)
                      </SelectItem>
                      <SelectItem value={PositionType.ASSISTANT}>
                        Assistant (Naib)
                      </SelectItem>
                      <SelectItem value={PositionType.SPECIAL}>
                        Special (Muhasib, Murabiy)
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <FormMessage />
          ...
          <div className="mt-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Dila (Local Jamaat)</FormLabel>
                  <FormControl>
                    <Input
                  ...
                  {offices.map((office) => (
                    <label
                      ...
                      className={cn(
                        ...
                        field.value?.includes(office._id)
                          ? "bg-primary/10"
                          : "hover:bg-card",
                      )}
                    >
